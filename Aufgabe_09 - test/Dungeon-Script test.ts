...
// ----------- Funktionen ----------- //
window.onload = function ()                                                                                         // Generelle onload-funktion um Event-Listener zum Dokument hinzuzufügen
{
...
{
    let RandomMonster: number = getRNGNumber(3) + 1;                        //Generiert eine Zufällige Zahl zwischen 0-2+1
    ...
    for (let i: number = 0; i < RandomMonster; i++) {
        let newMonsterDifficulty : number = generateMonsterDifficulty();    // Eigens-gebaute Funktion, welche eine Zahl zurück gibt.
...
function monsterGenerateHTML()                                          // Generiert HTML-Elemente, welche dann einem Element untergeordnet werden. Erzeugt ebenfalls einen Event-Listener auf dem Button.
{
...
{
    let generatedMonsterName : string = "";                             // Erstelle einen leeren String für den Monsternamen
...
function generateMonsterHealthPoints() : number             // Wird für die Monster-Lebenspunkte aufgerufen.
{
...
function updatePlayerLevel()                            // Aufgerufen, um das HTML-Element, welches das Spieler-Level darstellt, zu erneuern.
{
