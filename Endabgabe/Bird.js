...
(function (Vogelhaus_Predator) {
    class Bird extends Vogelhaus_Predator.Movable {
        constructor() {
            super();
        ...
        static getCloseBirds() {
            let gameEngine = Vogelhaus_Predator.GameEngine.instance();
            if (gameEngine.birdFood == null) {
                return [];
            }
            var result = [];
            for (var i = 0; i < gameEngine.drawables.length; i++) {
                var drawable = gameEngine.drawables[i];
                if (drawable instanceof Bird) {
                    if (drawable.isCloseTo(gameEngine.birdFood, 10)) {
        ...
        isCloseTo(_other, _maxDistance) {
            let _vec = Vogelhaus_Predator.Vector.fromPoints(this.position, _other.position);
            return _vec.length() <= _maxDistance;
        }
        update(_timeslice) {
            let isInDanger = this.isCloseToSnowball();
            let isMovingToFood = this.isCloseToBirdFood();
            let isEating = this.isEating();
            // Vektor
            if (isInDanger) {
            ...
            if (isEating && isInDanger) { //Der Vogel ist am Essen, will aber fliehen - also flieht er sehr langsam
                ...
                this.velocity = Vogelhaus_Predator.Vector.identity();
            }
